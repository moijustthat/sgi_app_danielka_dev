...
const ProductosBD = (props) => {
    ...
    const subtotal = truncarDecimal(lista.reduce((a, b) => Number(a) + Number(b['Cantidad']) * Number(b['Precio de venta']), 0))
    const total = truncarDecimal(lista.reduce((a, b) => Number(a) + calcularTotal(Number(b['Cantidad']), Number(b['Precio de venta']), Number(b['Cantidad con descuento']), Number(b['Porcentaje de descuento'])), 0))
    ...
    const productosFiltrados = (
        productos
            .filter(({ info }) => {
                const cantidadDisponible = Number(info['Disponible'])
                ...
                if (checkFilter && nameFilter && codeFilter && cantidadDisponible > 0) return true
                return false
            })
            .map(({ info }) => {
                ...
                return (<CardView
                    ...
                    replaceBtn={cantidad > 0 ?
                        <Adder
                            onPlus={() => addOnList(setBothLists, cantidad, 'Cantidad', info.id, info['Disponible'])}
