...
const Ordenes = () => {
  ...
  const permisoCrearOrdenes =
    permisos.find((p) => p.moduloId == 2) &&
    permisos.find((p) => p.moduloId == 2).estado === "t"
      ? true
      : false;
  const permisoLeerOrdenes =
    permisos.find((p) => p.moduloId == 3) &&
    permisos.find((p) => p.moduloId == 3).estado === "t"
      ? true
      : false;
  ...
  const orderExecInput = (id) => {
    setLoading(true);
    axiosClient
      .get(`/entrada/orden/${id}`)
      .then(({ data }) => {
        const entrada = data.entrada;
        const orden = ordenes.find((o) => o.id === id);
        for (let e of entrada) {
          const producto = productos.find(
            ({ info }) => String(info.id) === String(e.productoId)
          ).info;
          e["Almacen"] = "empty";
          e["Fecha de vencimiento stock"] =
            producto["Caducidad"] === "t" ? "" : "No caduca";
        }
    ...
    if (currentOrden.id !== id) {
      axiosClient
        ...
        .then(({ data }) => {
          ...
          const factura = (
            <OrdenTemplate orden={orden} detalles={detalles} actions={false} />
          );
          setDetails(factura);
          setOpenDetails(true);
        })
      ...
      const factura = (
        <OrdenTemplate actions={false} orden={orden} detalles={detalles} />
      );
      setDetails(factura);
      setOpenDetails(true);
    }
  ...
  const getItems = async () => {
    ...
    axiosClient
      ...
      .then(({ data }) => {
        ...
        setUnidadesMedida(
          data.unidades_medida.map((medida, index) => {
            return {
              label: medida.nombre,
        ...
        content={
          <EntregaTemplate
            ...
            footer={[
              {
                label: "Total abonado",
                value: "C$ " + formatearNumeroDinero(entrada.orden["Pagado"]),
              },
            ]}
          />
        }
        ...
        content={
          <ItemsTemplate
            header={{
              ...
              row2RightLabel: "Fecha limite de pago",
              row2RightValue:
                abonos.orden["Fecha limite de pago"] || "Sin fecha fimite",
            }}
            ...
            footer={[
              {
                label: "Total abonado",
                value: "C$ " + formatearNumeroDinero(abonos.orden["Pagado"]),
              },
            ]}
          />
        }
      ...
      <>
        <div className="ListaOrdenes">
          ...
          <AlertDialog
            ...
            contentText={
              activar ? `Seguro deseas volver a activar esta orden?` : ""
            }
            cancelText="Cancelar"
            acceptText="Aceptar"
            acceptAction={() => {
