...
import {
  ...
  ImgField,
  DateField,
} from "../../../Common/AwesomeFields/AwesomeFields";
...
const CreateInvoice = React.memo((props) => {
  ...
  const initNewDetalle = {
    ...
    "Unidad de medida":
      unidades_medida.length > 0 ? unidades_medida[0].value : "empty",
    Metodo: "peps",
    Minimo: "",
    Maximo: "",
    Caducidad: "f",
  ...
  const formatPreOrden = (orden, proveedor, detalles) => {
    const formatedOrden = {...orden, 'Fecha de entrega': dateHandler.convertirFechaEspaniol(orden['Fecha de entrega'])};
    formatedOrden.id = "new";
    formatedOrden["Proveedor"] =
      proveedor.id === "new"
        ? proveedor["Razon social"]
        : proveedores.find((p) => String(p.value) === String(proveedor.id))
            .label;
    formatedOrden['Numero RUC'] =  proveedor['Numero RUT']
    formatedOrden["Fecha emision"] = dateHandler.getCurrentDate();
    formatedOrden["Hora emision"] = dateHandler.getCurentTime();
    formatedOrden["Estado entrega"] = 'Esperando'
    formatedOrden["Orden hecha por"] = currentUserName;
    formatedOrden["Subtotal"] = (
      detalles.reduce(
        (a, od) =>
          a + parseFloat(od["Precio de compra"]) * Number(od["Cantidad"]),
        0
      )
    ).toFixed(2);
    formatedOrden["Descuento"] = detalles
      .reduce(
        (a, od) =>
          a +
          (Number(od["Cantidad con descuento"]) *
            parseFloat(od["Precio de compra"]) *
            parseFloat(od["Porcentaje de descuento"])) /
            100,
        0
      )
      .toFixed(2);
    formatedOrden["Porcentaje"] = (formatedOrden["Descuento"]*100 / (formatedOrden['Subtotal'])).toFixed(2)
    formatedOrden["Cargos por mora"] = 0;
    formatedOrden["Total"] = (
      (formatedOrden["Subtotal"] - formatedOrden["Descuento"])*1.15
    ).toFixed(2);
  ...
  const handleChangeNuevoProducto = (
    value,
    setErr,
    key,
    validate,
    ...
    if (!validate(value)) {
      // Mostrar mensaje
      setErr(
        personalized !== "" ? personalized : `Entrada incorrecta: ${value}`
      );
      return;
    } else {
      setErr("");
  ...
  const handleAgregarNuevoDetalle = (detalle) => {
    ...
    for (let require of required) {
      if (
        !!!nuevoDetalle[require] ||
        nuevoDetalle[require] === "" ||
        nuevoDetalle[require] === "empty"
      ) {
        incompletes.push(require);
      }
    }
    ...
    if (nuevoDetalle["Cantidad con descuento"] === "")
      nuevoDetalle["Cantidad con descuento"] = 0;
    if (nuevoDetalle["Porcentaje de descuento"] === "")
      nuevoDetalle["Porcentaje de descuento"] = 0;
    ...
    return (
      <>
        <div className="container">
          ...
          <div
            className={`glass ${listFullSize ? "fullGlass" : "partialGlass"}`}
          >
            <div className="exit">
            ...
            <div className="formCarrito">
              <div>
                <div className="mainData">
                  <div className="TitleContainer">
                    ...
                    <p>
                      *Ingrese la informacion del proveedor al que pedira la
                      orden
                    </p>
                  </div>
                </div>
              ...
              <div>
                <div className="mainData">
                  <SelectField
                    ...
                    label="Proveedor*"
                    options={[
                      { value: "new", label: "Nuevo proveedor" },
                      ...proveedores,
                    ]}
                    onChange={(value) => {
                      setProveedor({
                ...
                <div style={{ display: proveedor.id === "new" ? "" : "none" }}>
                  <div className="secondaryData">
                    <TextField
                      ...
                      label="Razon social"
                      incomplete={markAsIncomplete.find(
                        (l) => l === "Razon social"
                      )}
                      onChange={(value, setErr, setWarning) => {
                        handleFoundCostValidation(
                          ...
                          () => {
                            setWarning(
                              "Proveedor encontrado en la base de datos"
                            );
                  ...
                  <div className="secondaryData">
                    <TextField
                      ...
                      onChange={(value, setErr, setWarning) => {
                        handleConditionalCostValidation(
                          value,
                          (correo) => validateApi.email(correo),
                          () => {
                            setWarning("");
                            handleRollbacks(
                              setRollbacks,
                    ...
                    <TextField
                      ...
                      onChange={(value, setErr, setWarning) => {
                        handleFoundCostValidation(
                          ...
                          () => {
                            setWarning(
                              "Telefono encontrado en la base de datos"
                            );
              ...
              <div>
                <div className="secondaryData">
                  <DateField
                    ...
                    onChange={(value, setErr, setWarning) => {
                      // Lambda rollback
                      if (
                        dateHandler.isGreater(
                          value,
                          orden["Fecha de pago limite"]
                        )
                      ) {
                        setWarning(
                          "La fecha de entrega no puede ser despues de haber cobrado mora"
                        );
                        setRollbacks({
                          ...rollbacks,
                  ...
                  <SelectField
                    ...
                    options={[
                      { value: "t", label: "Establecer fecha limite" },
                      { value: "f", label: "No establecer fecha limite" },
                    ]}
                    onChange={(value, setErr, setWarning) => {
                      setOrden({
                        ...orden,
                ...
                <div
                  style={{
                    display:
                      orden["Establecer limite de pago"] == "t" ? "" : "none",
                  }}
                  className="secondaryData"
                >
                  <DateField
                    ...
                    onChange={(value, setErr, setWarning) => {
                      // Lambda rollback
                      if (
                        dateHandler.isLesser(value, orden["Fecha de entrega"])
                      ) {
                        setWarning(
                          "La fecha limite no puede ser anterior a la de entrega"
                        );
                        setRollbacks({
                          ...rollbacks,
                          "Fecha limite de pago": true,
                  ...
                  <TextField
                    ...
                    onChange={(value, setErr, setWarning) => {
                      if (
                        validateApi.decimalPositiveOrZero(value) &&
              ...
              <div className="mainData">
                <SelectField
                  ...
                  label="Producto"
                  options={[
                    { value: "new", label: "Nuevo producto" },
                    { value: "bd", label: "Buscar en la base de datos" },
                  ]}
                  onChange={(value, setErr) => {
                    if (value === "bd") {
                      setRequestBd(
              ...
              <div style={{ display: nuevoDetalle.id === "new" ? "" : "none" }}>
                    ...
                    onChange={(value, setErr, setWarning) => {
                      ...
                      if (!edit)
                        handleFoundCostValidation(
                          ...
                          () => {
                            setWarning(
                              "Producto encontrado en el catalogo de productos"
                            );
                            setRollbacks({
                              ...rollbacks,
                              Nombre: true,
                      ...
                      handleChangeNuevoProducto(
                        value,
                        setErr,
                        "Nombre",
                        validateApi.everything
                      );
                    }}
                  />
                ...
                <div className="secondaryData">
                  <div
                    style={{
                      display:
                        nuevoDetalle["Categoria"] === "new" ? "none" : "",
                    }}
                  >
                    <SelectField
                      label="Categoria"
                      ...
                      options={myConcat(
                        [{ label: "Nueva categoria", value: "new" }],
                        categorias
                      )}
                      onChange={(value, setErr, setWarning) => {
                        setNuevoDetalle({
                  ...
                  <div
                    ...
                    className="campoNuevaCategoria nuevoItem"
                  >
                    <div className="secondaryData">
                      <TextField
                  ...
                  <div
                    style={{
                      display: nuevoDetalle["Marca"] === "new" ? "none" : "",
                    }}
                  >
                    <SelectField
                      label="Marca"
                  ...
                  <div
                    ...
                    className="campoNuevaMarca nuevoItem"
                  >
                    <div className="secondaryData">
                      <TextField
                        value={items.Marca}
                ...
                <div className="secondaryData">
                  <div
                    style={{
                      display:
                        nuevoDetalle["Unidad de medida"] === "new"
                          ? "none"
                          : "",
                    }}
                  >
                    <SelectField
                      ...
                      incomplete={markAsIncomplete.find(
                        (l) => l == "Unidad de medida"
                      )}
                      options={myConcat(
                        [{ label: "Nueva medida", value: "new" }],
                        unidades_medida
                      )}
                      onChange={(value, setErr, setWarning) => {
                        setNuevoDetalle({
                    ...
                    <div className="secondaryData">
                      ...
                      <div className="btnGroup">
                        <button
                          onClick={() => {
                            const payload = {
                              medida: items["Unidad de medida"],
                            };
                            axiosClient
                              .post("/unidad_medida", payload)
                              .then(({ data }) => {
                        ...
                        handleDoubleCostValidation(
                          ...
                          (val1, val2) => {
                            if (val1 > val2) {
                              ...
                              setWarning(
                                "Debes dejar un margen entre el maximo y minimo"
                              );
                              return true;
                            } else {
                              setWarning("");
              ...
              <div
                style={{ display: nuevoDetalle.id === "new" ? "" : "none" }}
                className="secondaryData"
              >
                <TextField
                  label="Cantidad"
                  ...
                  onChange={(value, setErr, setWarning) => {
                    ...
                    handleDoubleCostValidation(
                      { label: "Cantidad", value: cantidad },
                      {
                        ...
                        value: Number(nuevoDetalle["Cantidad con descuento"]),
                      },
                      (val1, val2) => {
                        if (val1 < val2) {
              ...
              >
                <TextField
                  ...
                  onChange={(value, setErr, setWarning) => {
                    const cantidad = Number(value);
                    handleDoubleCostValidation(
                      { label: "Cantidad con descuento", value: cantidad },
                      {
                        label: "Cantidad",
                        value: Number(nuevoDetalle["Cantidad"]),
                      },
                      (val1, val2) => {
                        if (val1 > val2) {
                          setWarning(
                            "Esta cantidad sobrepasa a la de tu orden"
                          );
                          return true;
                        } else {
                          setWarning("");
              ...
              <TablaNuevaOrden
                ...
                footer={
                  !!!listFullSize ? (
                    ...
                    >
                      <h4>Despliega para ver la pre orden</h4>
                      <label>
                        <UilExpandAlt />
                      </label>
                    </div>
            ...
            <button
              ...
              className={`btnAgregarItem ${
                !listFullSize ? "partialBtn" : "noneBtn"
              }`}
            ...
            <button
              onClick={() => onRealizarOrden(listaDetalles, proveedor, orden)}
              className={`btnAgregarOrden ${
                !listFullSize ? "partialBtn" : "fullBtn"
              }`}
            >
              Realizar orden
            </button>
